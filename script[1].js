...
if(welcomeScreen){
  welcomeScreen.addEventListener('click', ()=>show('home'));
...
// Return to the welcome screen after 90 seconds of inactivity.
let timer;
function resetTimer(){
  clearTimeout(timer);
  timer = setTimeout(()=>show('welcome'), 90000);
}
...
function answerZooFitness(question){
  ...
  if(/\b(under 16|15|14|13|12|11|10|child|minor|son|daughter|parent|guardian|mum|mom|dad)\b/.test(q) && /\b(join|member|gym|allowed|age|old enough|come in|with me|accompanied)\b/.test(q)){
    return "No. Zoo Fitness is strictly 16+. Anyone using the gym must be aged 16 or over. Under-16s cannot join or use the gym, even if accompanied by a parent or guardian.";
  }
  if(/\b(age|old enough|minimum age)\b/.test(q)){
    return "The minimum age to join and use Zoo Fitness is 16. Under-16s cannot join or use the gym, even if accompanied by a parent or guardian.";
  }
  ...
  if(/\b(voucher|vouchers|gift voucher|gift vouchers|gift card|gift cards)\b/.test(q)){
    return "Yes, we do vouchers. Please speak to a staff member, choose Need Staff Help to contact us by WhatsApp, use the Ring doorbell, or email zoofitnessardee@gmail.com.";
  }
  ...
  if(/\b(parking|park my car|car park)\b/.test(q)){
    return "Yes. Parking for Zoo Fitness members is available at both the front and the rear of the Ardee Enterprise Centre.";
  }
