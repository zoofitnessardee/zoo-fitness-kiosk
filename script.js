...
function answerZooFitness(question){
  ...
  // Safety / privacy boundaries first
  ...
  if(/\b(cancel|cancellation|freeze|frozen|refund|charged|payment problem|payment issue|complaint|dispute)\b/.test(q)){
    return "That needs member-account or staff support. Please use your ClubRight member account, or choose Need Staff Help for WhatsApp, the Ring doorbell or a staff member.";
  }

  // Prices and memberships
  if(/\b(price|prices|membership price|membership prices|cost|how much|rates)\b/.test(q)){
    if(/\b(day|day pass)\b/.test(q)) return "A day pass is €10. Tap Membership Prices if you’d like to continue.";
    if(/\b(week|1 week|one week|weekly)\b/.test(q)) return "A 1-week pass is €25. Tap Membership Prices if you’d like to continue.";
    if(/\b(1 month|one month|monthly)\b/.test(q) && !/\b(recurring|direct debit|repeat|ongoing)\b/.test(q)) return "A 1-month membership is €39.";
    if(/\b(3 month|three month)\b/.test(q)) return "3 months is €100.";
    if(/\b(6 month|six month)\b/.test(q)) return "6 months is €190.";
    if(/\b(12 month|twelve month|year|annual)\b/.test(q)) return "12 months is €345.";
    if(/\b(recurring|ongoing|monthly recurring|direct debit)\b/.test(q)) return "The recurring monthly membership is €35 per month plus a €4 initial fee. For privacy, we recommend completing that signup on your own phone using the QR code shown under Membership Prices.";
  ...
  if(/\b(day pass|single day)\b/.test(q)) return "A day pass is €10. Open Membership Prices and tap Day pass to continue.";
  if(/\b(week pass|one week|1 week)\b/.test(q)) return "A 1-week pass is €25. Open Membership Prices and tap 1 week to continue.";
  if(/\b(recurring|direct debit|ongoing monthly)\b/.test(q)) return "The recurring monthly option is €35 per month plus a €4 initial fee. Please use Membership Prices and choose Recurring Monthly Membership; we recommend finishing that signup on your own phone.";
  ...
  if(/\b(age|old enough|16|15|14|17|minor|teen)\b/.test(q)){
    return "The minimum joining age at Zoo Fitness is 16.";
  }
  ...
  // Classes / PT / facilities — unknown details: escalate rather than invent
  if(/\b(class|classes|timetable|pt|personal trainer|personal training|shower|showers|parking|wifi|sauna|locker|changing room|guest|bring a friend)\b/.test(q)){
    return "I don’t have a confirmed Zoo Fitness answer for that yet, so I won’t guess. Please choose Need Staff Help and the team can help you.";
  }
